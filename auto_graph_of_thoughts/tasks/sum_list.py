...
def validate_op_split(previous_state: State, current_state: State) -> bool:
    """
    ...
    :return:
    """
    min_tolerated_sublist_ratio = 0.25
    if 'list' in previous_state and 'lists' in current_state:
        previous_list: Sequence[int] = previous_state['list']
        current_lists: Sequence[Sequence[int]] = current_state['lists']
        sum_correct =  sum(sum(l) for l in current_lists) == sum(previous_list)
        sublist_ratio = 1 - (abs(len(current_lists[0]) - len(current_lists[1])) / (len(previous_list) / 2))
        return sum_correct and sublist_ratio >= min_tolerated_sublist_ratio
    return False

def score_op_split(cumulative_score: float, previous_state: State, current_state: State) -> float:
    """
    ...
    if cumulative_score < 0.0:
        return -1.0
    if validate_op_split(previous_state, current_state):
        return 1.0
    return -1.0
...
def validate_op_merge(previous_state: State, current_state: State) -> bool:
    """
    ...
    :return:
    """
    if 'lists' in previous_state and 'list' in current_state:
...
def score_op_merge(cumulative_score: float, previous_state: State, current_state: State) -> float:
    """
    Determines the score of the split operation.
    :param cumulative_score: cumulative score
    :param previous_state: previous state
    ...
    if cumulative_score < 0.0:
        return -1.0
    if validate_op_merge(previous_state, current_state):
        return 1.0
    return -1.
